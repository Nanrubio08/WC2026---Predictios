...
import { registerUser, loginUser, googleAuth, forgotPassword } from '../services/api';
          ...
          {mode === 'forgot' ? (
            <>
              <div className="mb-6 text-center">
                <button onClick={() => switchMode('login')} className="mb-4 flex items-center gap-1 text-xs text-wc-muted hover:text-wc-text transition-colors" style={{ fontFamily: 'Barlow Condensed, sans-serif', letterSpacing: '0.1em' }}>
              ...
              {forgotSent ? (
                <div className="rounded-xl px-5 py-6 text-center space-y-3" style={{ background: 'rgba(0,200,122,0.06)', border: '1px solid rgba(0,200,122,0.25)' }}>
                  <div className="text-3xl">📬</div>
                  <p className="font-bold text-sm" style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '1rem', color: '#00C87A', letterSpacing: '0.04em' }}>
                    ¡Correo enviado!
                  </p>
                  <p className="text-sm text-wc-muted">
                    Si existe una cuenta con ese correo, recibirás un enlace válido por <strong className="text-wc-text">1 hora</strong>. Revisa también tu carpeta de spam.
                  </p>
                  <button onClick={() => switchMode('login')} className="btn-outline mt-2 px-6 py-2 text-sm">
                    Volver al inicio de sesión
                  </button>
                </div>
              ) : (
              ...
              <form onSubmit={handleSubmit} className="space-y-4">
                ...
                <div>
                  <div className="mb-1 flex items-center justify-between">
                    <label className="block text-xs font-bold uppercase tracking-wider text-wc-muted"
                      style={{ fontFamily: 'Barlow Condensed, sans-serif', letterSpacing: '0.12em' }}>Contraseña</label>
                    {mode === 'login' && (
                      <button type="button" onClick={() => switchMode('forgot')}
                        className="text-xs transition-colors hover:text-wc-text"
                        style={{ fontFamily: 'Barlow Condensed, sans-serif', color: '#F5A623', letterSpacing: '0.06em' }}>
